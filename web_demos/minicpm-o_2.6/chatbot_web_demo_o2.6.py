...
if args.multi_gpus:
   ...
   # Now load the model with empty weights
   with init_empty_weights():
       model = AutoModel.from_pretrained(local_model_path, trust_remote_code=True, attn_implementation='sdpa', torch_dtype=torch.bfloat16,
           init_audio=False, init_tts=False)
   
   # Adjust memory allocation for your GPUs
   ...
   # Load the checkpoint
   model = load_checkpoint_and_dispatch(model, local_model_path, dtype=torch.bfloat16, device_map=device_map)
else:
   # Simple single-GPU loading (will likely OOM with 16GB VRAM)
